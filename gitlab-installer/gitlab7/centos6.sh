...
if [ ! -f epel-release-6-8.noarch.rpm ]; then
    curl -O http://dl.fedoraproject.org/pub/epel/6/i386/epel-release-6-8.noarch.rpm || exit 1
fi
rpm -ivh epel-release-6-8.noarch.rpm
sed -i "s/enabled=1/enabled=0/" /etc/yum.repos.d/epel.repo

yum -y groupinstall "development tools"
yum -y --enablerepo=epel install zlib-devel openssl-devel gdbm-devel readline-devel ncurses-devel libffi-devel libxml2-devel libxslt-devel libcurl-devel libicu-devel libyaml-devel logrotate expect cmake wget
...
if [ ! -x /usr/bin/ruby ]; then
    cd /usr/local/src
    curl -O http://ftp.ruby-lang.org/pub/ruby/ruby-2.1.5.tar.gz || exit 1
...
if [ ! -x /usr/bin/gem ]; then
    cd /usr/local/src
    curl -OL http://production.cf.rubygems.org/rubygems/rubygems-2.4.4.tgz || exit 1
    tar xvzf rubygems-2.4.4.tgz
    cd rubygems-2.4.4
...
if [ ! -d gitlab-shell ]; then
    ...
    sed -i 's#^gitlab_url: "http://localhost:8080/"$#gitlab_url: "http://$HOSTNAME:8000/"#' /home/git/gitlab-shell/config.yml
	sed -i "s/socket: \/var\/run\/redis\/redis.sock/socket: \/tmp\/redis.sock/" /home/git/gitlab-shell/config.yml
...
if [ ! -d gitlab ]; then
	...
	sed -i "s/port: 80/port: 8000/" config/gitlab.yml
    sed -i "s/email_from: gitlab@localhost/email_from: gitlab@$HOSTNAME/" config/gitlab.yml
	sed -i "s/listen \"127.0.0.1:8080\", :tcp_nopush => true/listen \"$HOSTNAME:8000\", :tcp_nopush => true/" config/unicorn.rb
	...
	sudo -u git git config --global core.autocrlf "input"
...
wget -O /etc/nginx/conf.d/gitlab.conf https://raw.github.com/akagisho/gitlab-installer/master/nginx.conf || exit 1
